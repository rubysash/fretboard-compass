...
def get_alternative_fingerings(chord_name: str, chord_notes: List[str], start_fret: int) -> List[Dict[str, Any]]:
    ...
    if std_f is not None:
        options.append({"style": "Standard", "fingering": std_f, "states": std_s, "fret": 1})

    # 2. Positional (Solver-based at the current start_fret)
    pos_f, pos_s = get_best_chord_fingering(chord_name, chord_notes, start_fret)
    options.append({"style": f"Positional (fr {start_fret})", "fingering": pos_f, "states": pos_s, "fret": start_fret})

    # 3. Barred / Higher (Look at next logical position if start_fret is low)
    if start_fret < 5:
        high_f, high_s = get_best_chord_fingering(chord_name, chord_notes, 5)
        options.append({"style": "Alternative (fr 5)", "fingering": high_f, "states": high_s, "fret": 5})
