...
chords format:
VERIFIED	KEY	NAME	POSITION	DOTS
1	XX3211	A/C#	5	[320,160,3],[340,130,2],[360,100,1],[380,100,1]
1	133211	A#maj	6	[280,100,1],[300,160,3],[320,160,4],[340,130,2],[360,100,1],[380,100,1]
1	X13321	A#min	1	[300,100,1],[320,160,3],[340,160,4],[360,130,2],[380,100,1]
1	133111	A#min	6	[280,100,1],[300,160,3],[320,160,4],[340,100,1],[360,100,1],[380,100,1]


favs format:
VERIFIED	KEY	NAME	POSITION	DOTS
1	I II III iii D	DMaj	1	DMaj EMaj C#maj F#min
1	I iii IV V Ballads C	CMaj	1	CMaj Em FMaj GMaj

scales format example:
VERIFIED	KEY	NAME	POSITION	DOTS
1	AMaj	AMaj	1	[0,2,4],[100,2,4],[0,2,4],[1,102,4],[0,2,3],[0,2,4]
1	BMaj	BMaj	1	[0,2,4],[1,102,4],[1,2,4],[1,3,104],[100,2,4],[0,2,4]


The idea is to show exactly what to play for both scales and progressions
for my favorite progressions (and all progressions)
...
Todo:
Move svg and html into container folders instead of in the root
Add more data to the chord database (full scales, full progressions, not just my favorites
...
Does not allow duplicate fingerings of different chords (D#m/Eb,Abm/G#m etc)
...
ALLFILE = f"{VERSION}.all.html"
FAVFILE = f"{VERSION}.favs.html"
SCALESFILE = f"{VERSION}.scales.html"
...
def open_html(htmlfile):
    ...
    Args:
        htmlfile (str): The path to the HTML file to open.
    """
    if os.name == "nt":
...
def get_raw(sheet):
    """Extract raw data from a worksheet into a dictionary.
...
def dump_favs(ffile, favorites, scales, chords):
    """Generate an HTML file for favorite chord progressions.

    Args:
        ffile (str): The filename for the output HTML.
        favorites (dict): A dictionary of favorite chord progressions.
        scales (dict): A dictionary of scales.
        chords (dict): A dictionary of chords.

    # All are lists of dictionaries:
    ...
    "Pachel D_1": {
      ...
      "name": "DMaj",
      "position": 1,
      "dots": "DMaj AMaj Bm F#min GMaj DMaj GMaj AMaj"
    },
    ...
    with open(ffile, "w", encoding="utf-8") as favs:
        ...
        for fav_key in favorites:
            fav_name, _ = fav_key.split("_")
            favs.write(f"<h3>{fav_name}</h3>\n<table>\n")
            dot = favorites[fav_key]["dots"]
            key_name = favorites[fav_key]["name"]

            # Retrieve the scale SVG if it exists
            scale_key = f"{key_name}_{favorites[fav_key]['position']}"
            if scale_key in scales:
                scalesvg = (
                    f"{scales[scale_key]['key']}_{scales[scale_key]['position']}.svg"
                )
                favs.write(
                    f"<tr><td><img src='{scalesvg}' alt='{key_name} scale' /></td>\n"
                )
            ...
            for dot in dots:
                ...
                matching_chords = [
                    chord for chord in chords.values() if chord["name"] == dot
                ]
                # Sort them by position, preferring '1' if it exists
                matching_chords.sort(
                    key=lambda c: (c["position"] != "1", c["position"])
                )
                if matching_chords:
                    chord_key = matching_chords[0]["key"]
                    chord_position = matching_chords[0]["position"]
                    chord_svg = f"{chord_key}_{chord_position}.svg"
                    if columns == 4:
                        favs.write(
                            "</tr>\n<tr>"
...
def get_data(inputfile, data):
    ...
    try:
        ...
        # Your code to process the sheets
...
def main():
    ...
    inputfile = ""
    dump_options = {"-j": False, "-c": False, "-f": False, "-s": False, "-a": False}
