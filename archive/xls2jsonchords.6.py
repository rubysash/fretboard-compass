# for glob stuff that lists all files in a directory
import glob

# for argv and sys.exit
import sys, getopt

# for clear screen function and webbrowsser chrome bug
...
# to create svg graphics
import svgwrite

# to read/write json
import json

# to sort dictionary and maintain order
import collections

# to open automagically in browser
import webbrowser
...
#scale   = 82  # 82 for printing, 67 for CNC, fixme:  not used
#dotsize = .15 # between .12 and .2 probably, fixme: not used
#stroke  = 3   # should probably be an odd number so it's centered, probably 5 or 7 fixme:  not usedd
xoffset = -260  # because I already had data in the wrong spot so I just globally moved it
yoffset = -40   # because I already had data in the wrong spot so I just globally moved it
...
#data    = {}                           # as we build dict of dict, this stores it globally
allfile = '6.all.html'
favfile = '6.favs.html'
s_file  = '6.scales.html'
# fixme:  (everywhere)   All of this (or most) could be put in separate module
# instead of having all these junk functions in the main logic
...
def show_html(filetype, d, htmlfile):
    """
    ...
    with open(htmlfile, 'w') as chords:
    ...
    webbrowser.get("C:/Program Files/Google/Chrome/Application/chrome.exe %s").open('file://' + os.path.realpath(htmlfile))
...
def dump_favs(ffile,favorites,scales,chords):
    ...
    with open(ffile, 'w') as favs:
        
        favs.write(header)

        for k in favorites:
            # fixme:  grabs last chord found only if duplicates
            found = {}      # used to eliminate multiple variations of chord
            founds = []
            good_name = k.split("_")
            ...
            # write out the list we built above as a table containing our svg
    ...
    # auto open in default browser
    webbrowser.get("C:/Program Files/Google/Chrome/Application/chrome.exe %s").open('file://' + os.path.realpath(ffile))
    ...
    for key in scales:
        ...
        if (key == None):
            ...
            if str(verified) == str(0):
                make_squares(0,0,screenx,screeny,1,'pink')  # the border # fixme: pink
            else:
            ...
            patterncount = 0
            for x in key:
                if x == '_': # was failing if key had zero in _position XXXXXX_10
                    break
                if x == 'X':
            ...
            if (len(fixedlist) > 1):
                ...
                for i in range(len(fixedlist)):
                    ...
                    for y in fret_pos:
                        ...
                        make_dots(xx,yy,8,color)           # this is the finger position, the actual dot
            else:
                pass
...
def dump_chords(chords):
    '''
    creates all fo the chords then calls show_html
    '''
        ...
        if (key == None):
            ...
            if (len(fixedlist) > 1):
                ...
                for i in range(len(fixedlist)):
                    ...
                    # fixme:  these do not scale, fix the offsets and radius to scale
                    make_dots(xx,yy,9,'white')           # this is the finger position, the actual dot
                    make_labels(xx-4,yy+4,'14px',text)   # this is the finger number inside the dot
            else:
                pass
    ...
    show_html('svg',d_sorted,allfile)
...
def main():
    ...
    for opt, arg in opts:
        ...
        if arg.startswith("-"):
            help_message("ARGUMENT OMITTED FOR '{}'".format(opt) + " (ie:  -i somefile.xlsx)")
